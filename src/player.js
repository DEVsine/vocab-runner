...
function buildAstronaut() {
  ...
  /* ── ซามูไร: คาตานะ + ฝักดาบ ─────────────────────────────── */
  {
    ...
    /* ⚠️ หมวก **เข้ม** ส่วนหน้า **แดง** — เคยทำกลับข้างกัน (หมวกแดง หน้ากากเหล็กเข้ม)
     * แล้วมันไม่เท่า เพราะหมวกแดงกับหน้าแดงแย่งกันเป็นพระเอก สุดท้ายไม่มีใครเด่น
     * ให้หมวกเป็นฉากหลังเข้ม ๆ แล้วปล่อยให้หน้ายักษ์แดงเป็นจุดเดียวที่ตาไปเกาะ
     * kabutoMat (แดง) ยังใช้กับกระโปรงเกราะอยู่ จึงต้องแยกวัสดุหมวกออกมาต่างหาก */
    const kabutoMat = toonMat(0xb2372e, { side: THREE.DoubleSide });
    const helmMat = toonMat(0x353b42, { side: THREE.DoubleSide });
    const tealMat = toonMat(0x1f7a78);
    const bowl = domeHelm(HEAD_R + 0.04, helmMat, 0.66);
    bowl.position.y = 0.025;
    helmG.add(bowl);
    gloss.push(glossShell(bowl, 86).material);

    /* แถบครามเขียวคาดหมวก — สีที่สามที่กันไม่ให้หมวกเข้มกลายเป็นก้อนดำตัน
     * ⚠️ ทอร์รัสเต็มวงพาดขวางใบหน้าพอดี (เจอมาแล้ว) — ต้องเป็นครึ่งวง **ด้านหลัง** เท่านั้น
     * thetaStart คุมว่าวงเริ่มตรงไหน ไม่ใช่ rotation — หมุนทั้งชิ้นแก้ไม่ได้เพราะวงมันปิด */
    const band = new THREE.Mesh(
      new THREE.TorusGeometry(HEAD_R + 0.028, 0.028, 6, 20, Math.PI * 1.1),
      tealMat,
    );
    ...
    /* ⚠️ เขาเคยเป็นกรวยผอม (รัศมี 0.052) ซึ่งคือสิ่งที่คอมเมนต์ข้างบนเตือนไว้เองว่าอย่าทำ
     * ที่ 150px มันหายไปเป็นเส้นขนสองเส้น เหลือแค่ "หมวกกลมสีแดง" ซึ่งไม่มีความเป็นซามูไร
     * ตอนนี้ทำเป็นแผ่นโค้งแบน (กว้าง 3 เท่า บาง) แบบมาเอดาเตะจริง ๆ
     * แผ่นแบนกว้างชนะกรวยผอมที่ระยะไกลเสมอ เพราะเงาทึบของมันมี "พื้นที่" ไม่ใช่แค่ "เส้น" */
    for (const side of [-1, 1]) {
      const hornG = new THREE.Group();
      // ท่อนล่างตั้งขึ้น + ท่อนบนสะบัดออก = เสี้ยวพระจันทร์ ไม่ใช่เขาสัตว์ตรง ๆ
      // เขาสีเข้มเหมือนหมวก ไม่ใช่ทอง — ทองไปกองอยู่ที่จานมงกลางหน้าผากจุดเดียวแล้ว
      // ของวาววับสองที่บนหัวเดียวกัน = ตาไม่รู้จะเกาะอะไร
      const lower = new THREE.Mesh(new THREE.BoxGeometry(0.058, 0.16, 0.17), helmMat);
      lower.position.set(0, 0.08, 0);
      lower.rotation.z = side * 0.22;
      hornG.add(lower);
      const upper = new THREE.Mesh(new THREE.ConeGeometry(0.095, 0.34, 4), helmMat);
      upper.scale.set(0.4, 1, 1.5);       // บีบให้แบน แล้วยืดตามแนวหน้า-หลัง
      upper.position.set(side * 0.1, 0.23, 0);
      upper.rotation.z = side * 1.02;     // กางออกเกือบนอน = เสี้ยวพระจันทร์ ไม่ใช่เขาชี้ฟ้า
      hornG.add(upper);
      // ⚠️ เคยอยู่ที่ y 0.2 แล้วเขาลอยหลุดเหนือหมวกเป็นใบมีดสองอันไม่ติดอะไรเลย
      // ต้องให้โคนเขาจมอยู่ในโดมหมวก ตาถึงจะอ่านว่า "งอกออกมาจากหมวก"
      hornG.position.set(side * 0.115, 0.075, -0.1);
      helmG.add(hornG);
    }
    // จานทองกลางหน้าผาก (มง) — จุดวาวจุดเดียวที่ดึงตาไปที่ใบหน้า
    // จานมง — ใหญ่ขึ้นและมีซี่รัศมี ให้เป็น "ตราประจำตัว" ไม่ใช่หมุดกลม ๆ
    const mon = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.075, 0.035, 16), weaponMat);
    mon.rotation.x = Math.PI / 2;
    mon.position.set(0, 0.185, -0.29);
    helmG.add(mon);
    for (let i = 0; i < 8; i++) {
      const ray = new THREE.Mesh(new THREE.BoxGeometry(0.026, 0.055, 0.02), weaponMat);
      const a = (i / 8) * Math.PI * 2;
      ray.position.set(Math.sin(a) * 0.088, 0.185 + Math.cos(a) * 0.088, -0.283);
      ray.rotation.z = -a;
      helmG.add(ray);
    }
    ...
    /* ── เมนโป (面頬): หน้ากากเหล็กหน้ายักษ์ ────────────────────────
     * ⚠️ คลุม "ครึ่งล่าง" เท่านั้น ห้ามคลุมตากับคิ้ว
     * ของจริงก็ทำแบบนี้ (ฮันโบ/โฮอาเตะ) และมีเหตุผลทางดีไซน์ด้วย:
     * คิ้วหนาเอียงเข้าหากันคือสิ่งเดียวที่ทำให้หน้าตัวนี้ "มีอารมณ์"
     * ปิดทั้งหน้าเมื่อไหร่ก็เหลือแค่วัตถุ ไม่ใช่ตัวละคร (ลอร์ดมืดรอดมาได้
     * เพราะแลกด้วยตาเรืองแสง ซึ่งตัวปั้นเองไม่มี)
     *
     * เขี้ยวชี้ขึ้น + หนวดสะบัดออก = อ่านว่า "ยักษ์" ไม่ใช่ "คนใส่ผ้าปิดปาก"
     * ทั้งสองอย่างต้องยื่นพ้นเส้นรอบรูปของหัว ไม่งั้นหายที่ 150px */
    /* ── หน้ายักษ์: ไม่ใช้หน้ากากครอบ แต่ "ย้อมหน้าเป็นสีแดง" แล้วเติมขนขาว ──
     *
     * ⚠️ รอบก่อนผมแปะหน้ากากเหล็กเข้มทับหน้าคน ผลคือมันอ่านเป็น "เครายาว"
     * เพราะก้อนเข้มบนหน้าสว่างอ่านเป็นขน ไม่ใช่โลหะ ไม่ว่าจะปั้นเหลี่ยมแค่ไหน
     *
     * ทางที่ถูกกว่าคือกลับด้าน: ให้ "หน้า" เป็นสีแดงทั้งใบ (ผ่าน face.tone)
     * แล้ววางของ **สีอ่อน** ทับ — คิ้วขาว หนวดขาว เครายาว
     * ได้หน้ายักษ์โดยเพิ่มชิ้นส่วนน้อยกว่าเดิม และไม่มีชิ้นไหนที่อ่านผิดได้
     * หลัก: ขนสีอ่อนบนหน้าสีเข้ม อ่านว่า "ขน" เสมอ · ก้อนสีเข้มบนหน้าสีอ่อน อ่านว่าอะไรก็ได้ */
    const hairMat = toonMat(0xf4eee2);

    // จมูกยักษ์ — ปุ่มกลมกลางหน้า จุดที่ทำให้หน้าแบนมี "ความลึก"
    const nose = new THREE.Mesh(new THREE.SphereGeometry(0.055, 14, 12), toonMat(0xa8281f));
    nose.scale.set(1, 0.9, 0.85);
    nose.position.set(0, -0.055, -0.352);
    helmG.add(nose);

    /* หนวดขาว — ปีกสองข้างสะบัดลง กว้างพ้นแก้ม
     * ⚠️ ต้องเป็นทรงเรียวปลาย ไม่ใช่กล่อง — รอบก่อนใช้กล่องยาวแล้วอ่านเป็นแท่งดำ
     * ปลายเรียวคือสิ่งที่บอกว่ามันเป็นขน ไม่ใช่ชิ้นส่วนแข็ง */
    for (const side of [-1, 1]) {
      const stache = new THREE.Mesh(new THREE.ConeGeometry(0.055, 0.2, 6), hairMat);
      stache.scale.set(1, 1, 0.5);
      stache.position.set(side * 0.072, -0.145, -0.322);
      // ⚠️ เคยตั้ง z = 2.35 (≈134°) แล้วหนวดชี้ออกเกือบนอน อ่านเป็นลูกศรสองอันปักหน้า
      // 2.75 (≈158°) = ปลายตกลงล่างชัดเจน ได้หนวดห้อยแบบในภาพอ้างอิง
      stache.rotation.set(-0.3, 0, side * 2.75);
      helmG.add(stache);
    }

    // เคราใต้คาง — ปิดเส้นขากรรไกร ทำให้หน้าดูหนักและแก่กว่าเดิม
    const beard = new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.19, 8), hairMat);
    beard.scale.set(1, 1, 0.62);
    beard.rotation.x = Math.PI + 0.22;
    beard.position.set(0, -0.215, -0.3);
    helmG.add(beard);

    // ปากยิงฟันใต้หนวด — แถบมืดแคบ ๆ พอให้รู้ว่ามีปากอยู่ ไม่ต้องมีฟันให้รก
    const maw = new THREE.Mesh(new THREE.BoxGeometry(0.115, 0.036, 0.04), toonMat(0x2a0f0c));
    maw.position.set(0, -0.163, -0.336);
    helmG.add(maw);
    ...
    function buildKatana() {
      ...
      /* ⚠️ แถบนี้ต้อง **แคบกว่าตัวใบเสมอ** — เดิมกว้าง 0.045 ขณะที่ใบกว้าง 0.042
       * พอมองเฉียงจนเกือบเห็นใบด้านข้าง แถบก็บังเนื้อเหล็กจนมิด เหลือแต่แผ่นครีมทึบ
       * ซึ่งอ่านเป็น "ไม้พาย" ไม่ใช่ดาบ · และสีเดิมอมเหลือง (0xfff6dd) ยิ่งซ้ำเติม
       * ตอนนี้แคบกว่าใบ + ขาวอมฟ้า → เหล็กโผล่เป็นเนื้อ คมเป็นเส้นเรืองอยู่ข้างเดียว */
      const edge = new THREE.Mesh(
        new THREE.BoxGeometry(0.034, 0.94, 0.03),
     ...
     *
     * ⚠️ y เคยเป็น 0.4 ซึ่งทำให้ **มือไม่ได้จับดาบ** — holdR อยู่ที่ระดับฝ่ามือ
     * ส่วนด้ามดาบ (tsuka) อยู่ที่ y = -0.17 ในพิกัดของดาบ และยาวแค่ 0.32
     * ดันขึ้นไป 0.4 ก็เท่ากับยกทั้งด้ามพ้นมือไปครึ่งช่วงตัว เหลือดาบลอยเหนือกำปั้น
     * ค่าที่ถูกคือ +0.17 พอดี = เอา "กลางด้าม" ไปวางไว้ที่ฝ่ามือ
     * กฎ: จุดหมุนของอาวุธต้องเล็งที่ **ส่วนที่มือจับ** ไม่ใช่จุดกำเนิดของโมเดล */
    const katanaHold = buildKatana();
    katanaHold.g.position.set(0.035, 0.17, -0.03);
