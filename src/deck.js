...
/**
 * หยิบ "ชุดข้อสอบ" ออกมา count ข้อ — ห้ามซ้ำกันในรอบเดียวเด็ดขาด
 *
 * ⚠️ ห้ามใช้ pickWord ที่นี่: pickWord ถ่วงน้ำหนักด้วยกล่อง Leitner เพื่อ *สอน*
 * (คำที่ยังไม่แม่นจะถูกยกมาถี่กว่า) ซึ่งเป็นสิ่งที่ถูกต้องสำหรับการฝึก
 * แต่ผิดสำหรับการวัดผล — ข้อสอบที่เลือกข้อจาก "สิ่งที่คุณยังไม่รู้" คือข้อสอบที่
 * คะแนนต่ำเสมอโดยโครงสร้าง และเทียบระหว่างสองรอบไม่ได้เลย
 * ชุดข้อสอบจึงต้องสุ่มแบบเสมอภาคจากคลังทั้งกอง
 */
export function pickExamWords(deck, count, rand = Math.random) {
  const words = shuffle(deck.words.slice(), rand);
