...
/**
 * deck นี้ต้องเล่นแบบ "โหมดสอบ" ไหม
 *
 * ⭐ ตัดสินจาก **flag `exam: true` ในไฟล์ deck** ไม่ใช่จากชนิดของ deck
 * เพราะ "เป็นข้อสอบ" กับ "เป็นคำตรงข้าม" เป็นคนละเรื่องกันโดยสิ้นเชิง —
 * deck วิชาก็ประกาศตัวเป็นข้อสอบได้ และวันหนึ่งอาจมี deck คำตรงข้ามที่ตั้งใจให้ซ้อมเล่น ๆ
 * ถ้าผูกกับ type ไว้ วันนั้นจะต้องรื้อทุกจุดที่เรียก isExamDeck ใหม่หมด
 *
 * ยังคง isAntonymDeck ไว้เป็นค่าโดยปริยาย เพื่อไม่ให้ deck คำตรงข้ามของใคร
 * ที่ยังไม่ได้เติม flag กลายเป็นโหมดปกติแบบเงียบ ๆ (fail-safe ไปทางที่ปลอดภัยกว่า)
 */
export function isExamDeck(deck) {
  return deck?.exam === true || isAntonymDeck(deck);
}

/**
 * ชุดข้อสอบของ deck นี้ — **ทั้งกอง เรียงลำดับตายตัว**
 *
 * ⚠️ สองข้อห้ามที่ทำให้ฟังก์ชันนี้หน้าตาน่าเบื่อกว่าที่ควร และทั้งคู่จำเป็น:
 *
 * 1) ห้ามสุ่มลำดับ — checkpoint เก็บแค่ตัวเลข "ทำไปแล้วกี่ข้อ" ถ้าลำดับสุ่มใหม่ทุกรอบ
 *    "ข้อที่ 51" ของรอบนี้จะเป็นคนละคำกับ "ข้อที่ 51" ของรอบก่อน → ผู้เล่นจะเจอคำซ้ำ
 *    และคำที่ไม่เคยเจอเลยจนจบ ทั้งที่ตัวเลขบอกว่าทำครบแล้ว (พังเงียบสนิท ไม่มี error)
 *
 * 2) ห้ามใช้ pickWord — มันถ่วงน้ำหนักด้วยกล่อง Leitner เพื่อ *สอน* (คำที่ยังไม่แม่น
 *    โผล่ถี่กว่า) ซึ่งถูกสำหรับการฝึก แต่ผิดสำหรับการวัด: ข้อสอบที่เลือกข้อจาก
 *    "สิ่งที่คุณยังไม่รู้" คือข้อสอบที่คะแนนต่ำเสมอโดยโครงสร้าง เทียบสองรอบไม่ได้
 *
 * ใช้ allWords ก่อน words เสมอ — words อาจถูกกรองด้วยระดับที่เลือกในเมนู
 * แต่ข้อสอบต้องครอบคลุมทั้งชุด (ลำดับในไฟล์เรียงระดับ 1→10 อยู่แล้ว = ง่ายไปยาก)
 */
export function examWordList(deck) {
  return (deck?.allWords ?? deck?.words ?? []).slice();
}
