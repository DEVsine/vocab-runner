...
export function createUI(handlers) {
  ...
  function showDeath(info) {
    ...
    if (ex) {
      /* สามสถานะ และต้องแยกให้ขาด เพราะมันคือข่าวคนละแบบกันสำหรับผู้เล่น:
       *   ยังไม่ถึงจุดเซฟแรก → ข่าวร้าย ต้องบอกตรง ๆ ว่าเริ่มใหม่ พร้อมบอกว่าอีกกี่ข้อถึงจะเซฟได้
       *   ผ่านจุดเซฟมาแล้ว    → ข่าวดี บอกว่ากลับไปที่ไหน (จุดเซฟ ไม่ใช่ข้อที่เพิ่งตาย)
       *   ทำครบแล้ว          → ไม่ควรมาถึงจอนี้ แต่กันไว้ ไม่ให้ขึ้นข้อความที่คำนวณผิด
       *
       * ⚠️ ห้ามเขียนแค่ "เริ่มที่ข้อ N+1" แบบเดิม — ตอนนี้มันไม่จริงแล้ว
       * และคำโกหกที่ให้ความหวังผิด ๆ แย่กว่าข่าวร้ายที่ตรงไปตรงมา */
      const el = $('dead-exam');
      if (ex.at >= ex.total) {
        el.textContent = `ทำครบ ${ex.total} ข้อแล้ว`;
      } else if (ex.savedAt > 0) {
        el.textContent = `💾 ทำไปแล้ว ${ex.at}/${ex.total} ข้อ — เซฟไว้ที่ข้อ ${ex.savedAt} รอบหน้าเริ่มข้อ ${ex.savedAt + 1}`;
      } else {
        const left = ex.saveEvery - ex.at;
        el.textContent = `ทำไปแล้ว ${ex.at}/${ex.total} ข้อ — ยังไม่ถึงจุดเซฟแรก (ข้อที่ ${ex.saveEvery}) อีก ${left} ข้อเท่านั้น รอบหน้าเริ่มข้อ 1 ใหม่`;
      }
      // ยังไม่มีอะไรให้เซฟ = ข่าวร้าย ห้ามใช้กรอบสีเขียวของ "ข่าวดี" หลอกตา
      el.classList.toggle('is-lost', ex.savedAt === 0 && ex.at < ex.total);
    }
